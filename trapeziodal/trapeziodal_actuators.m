function [trajectory_list] = trapeziodal_actuators(actuator_list, constraints)
  num_actuators = length(actuator_list);
  
  #Get index of actuator with longest stroke and length of stroke
  [maxStroke, actIndex] = get_longest_stroke(actuator_list);
  
  #Compute total duration and acceleration duration from Vmax and Amax applicable to
  #to actuator with longest stroke
  [Ta, T, linear_segment_present] = get_durations(actuator_list, constraints, actIndex)
  
  #Now from above T and Ta compute Vi and Ai for all actuators
  [result_list] = get_actuators_vel_accel(actuator_list, T, Ta);
  
  [q, v, a, j, t] = compute_trajectory(result_list(1), linear_segment_present, Ta);
   trajectory_list(1).q = q;
   trajectory_list(1).v = v;
   trajectory_list(1).a = a;
   trajectory_list(1).j = j;
   trajectory_list(1).t = t;
   
     
  [q, v, a, j, t] = compute_trajectory(result_list(2), linear_segment_present, Ta);
   trajectory_list(2).q = q;
   trajectory_list(2).v = v;
   trajectory_list(2).a = a;
   trajectory_list(2).j = j;
   trajectory_list(2).t = t;
   
endfunction

function [longest_stroke, actuator_index] = get_longest_stroke(actuator_list)
  num_actuators = length(actuator_list)
  longest_stroke = 0;
  actuator_index = 0;
  for i=1:num_actuators;
    currentStroke = actuator_list(i).endPosition - actuator_list(i).startPosition;
    if(currentStroke > longest_stroke)
      longest_stroke = currentStroke;
      actuator_index = i;
    endif
  endfor
endfunction
